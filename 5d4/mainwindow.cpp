...
//设置表头
    ...
    headerList=header.split(QRegularExpression("\\s+"),Qt::SkipEmptyParts);//一个或多个空格、TAB等分隔符隔开的字符串
...
//设置表格数据
    ...
    for (i=1;i<rowCnt;i++)
    {
        ...
        tmpList=aLineText.split(QRegularExpression("\\s+"),Qt::SkipEmptyParts);//一个或多个空格、TAB等分隔符隔开的字符串分解为多个字符串
